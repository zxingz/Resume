\documentclass[]{Vishnu-Resume}


\begin{document}

\lastupdated

\namesection{Vishnu}{Srivastava}{\href{https://vishnu.the-software.dev}{ \urlstyle{same}\url{vishnu.the-software.dev}} \\
\href{mailto:vishnu-srivastava@hotmail.com.com}{vishnu-srivastava@hotmail.com} | \href{tel:+12266002928}{+1 226 600 2928}
}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%     COLUMN ONE
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{minipage}[t]{0.33\textwidth} 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     EDUCATION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Education} 

\subsection{University of Waterloo}
\descript{\href{https://uwspace.uwaterloo.ca/handle/10012/11732}{MMath Thesis in Computer Science}}
Major: Software Engineering \\
Guide: \href{https://cs.uwaterloo.ca/about/people/palencar}{Prof. Paulo Alencar}\\
\location{Waterloo, ON | CGPA : 87\% \\ Sep'14 - May'17}

\sectionsep
\sectionsep
\sectionsep
\subsection{Delhi Technological}
\subsection{University}
\descript{B.Tech Engineering Physics}
Major:  Robotics \& Info Systems \\
\location{New Delhi, India \\ May'09 - Sep'14}
\sectionsep

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     LINKS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Links} 
Website:// \href{https://vishnu.the-software.dev}{\custombold{www.the-software.dev}} \\
Github:// \href{https://github.com/zxingz}{\custombold{github.com/zxingz}} \\
LinkedIn://  \href{https://www.linkedin.com/in/zxingz/}{\custombold{linkedin.com/in/zxingz}} \\
\sectionsep


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     SKILLS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Skills}
\subsection{Programming}
\location{Over 10000 lines:}
Python \textbullet{} Java \textbullet{} C++/C  \textbullet{} JavaScript \textbullet{} Bash
\sectionsep

\subsection{Python}
Apache Airflow \textbullet{} Pandas \textbullet{} PySpark  \textbullet{} Django \textbullet{} PyTorch  \textbullet{} Spacy   \textbullet{} SQLAlchemy  \textbullet{} scikit-learn
\sectionsep

\subsection{JavaScript}
Bootstrap4 \textbullet{} jQuery \textbullet{} ReactDOM 
\sectionsep

\subsection{Data Visualization}
Tableau \textbullet{} Grafana \textbullet{} D3.js  \textbullet{} Plotly \textbullet{} Kibana
\sectionsep

\subsection{DB/Cache/QL}
 Postgresql  \textbullet{} Redis  \textbullet{} Hive  \textbullet{} MySQL  \textbullet{} Oracle
\sectionsep

\subsection{Dev/CI/CD}
Jenkins \textbullet{} Docker \textbullet{} Git \textbullet{} SVN
\sectionsep


\subsection{Monitoring}
ElasticSearch \textbullet{} Filebeat \textbullet{} CAdvisor \textbullet{} NodeExporter
\sectionsep

\subsection{Environments}
RHEL \textbullet{} Windows  \textbullet{} Hadoop HDFS \textbullet{} Apache Mesos-DC/OS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%     COLUMN TWO
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\end{minipage} 
\hfill
\begin{minipage}[t]{0.66\textwidth} 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     EXPERIENCE
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Experience (3 Year 11 Months)}
\runsubsection{Scotiabank}
\descript{}
\descript{Data Scientist | International Banking Analytics}
\location{August 2019 – Present | Toronto, ON}
\sectionsep
\sectionsep
\sectionsep
\sectionsep
\sectionsep
\sectionsep
\sectionsep
\sectionsep
\sectionsep
\sectionsep
\sectionsep
\sectionsep
\sectionsep
\sectionsep
\sectionsep
\begin{tightemize}
\item Working on customer segmentation into premium groups based on wealth and risk adjusted revenue.
\item Developed pricing simulation dashboard for Caribbean operations to simulate changes in fee structures.
\item Manage analytics pipeline to Scotiabank's Latin America and Caribbean businesses.
\end{tightemize}
\#\custombold{Microservices}  \#\custombold{DC/OS}  \#\custombold{Spark} \#\custombold{APIs}  \#\custombold{Data Visualization}
\sectionsep

\descript{Data Scientist | decision sciences}
\location{July 2018 – August 2019 | Toronto, ON}
\begin{tightemize}
\item Used Grafana and Postgres to create dashboards for Electronic Money Transfer transactions.
\item Analyze Electronic Money Transfers using PySpark to gather transaction information(Email ID, Acc No., IP) from log files on HDFS.
\item Developed Reading Comprehension System using Google Research's BERT model for SEDAR News documents.
\end{tightemize}
\sectionsep

\descript{Programmer Analyst | Centralized Risk Platform}
\location{July 2017 – July 2018 | Toronto, ON}
\begin{tightemize}
\item Deployed Apache Airflow for RMIT Risk Warehouse on Production environment with 80+ DAGs.
\item Deployed Elasticsearch and Filebeat for log analysis.
\item Deployed monitoring stack (Prometheus, Grafana, CAdvisor, NodeExporter).
\end{tightemize}
\sectionsep

\descript{Intern | Equity Trading \& Electronic Execution Services}
\location{May 2016 – August 2016 | Toronto, ON}
\begin{tightemize}
\item Instant Chat using Socket.IO, Email reporting/tracking server, Trading performance report.
\end{tightemize}
\sectionsep

\runsubsection{ERNST \& YOUNG L.L.P}
\descript{| Associate Consultant | IT Advisory}
\location{June 2013 – July 2014 | New Delhi, India}
\begin{tightemize}
\item Develop \& Design UI and Logic using SAP Advanced Business Application
Programming. Customize and Develop Data Report and Form Generation.
\end{tightemize}
\sectionsep

\runsubsection{SAMSUNG R\&D LAB}
\descript{| Intern | Data Analytics Group}
\location{May 2013 – June 2013 | New Delhi, India}
\begin{tightemize}
\item Extraction of movie trailer links from MovieLens data sets. UI for Samsung Smart TV gesture recognition Application.
\end{tightemize}
\sectionsep

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     Portfolio
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Project}
\runsubsection{\href{https://the-software.dev}{the-software.dev}} (In-Progress)
\location{\href{https://the-software.dev/u/vishnu}{ \urlstyle{same}\url{the-software.dev/u/vishnu}}}
\sectionsep
\begin{tightemize}
\item Application Streaming service for developers to showcase their working applications live through our streaming platform. It enables users to host their applications wherever they like (Cloud/ Personal Desktop) and share it with the world.
\end{tightemize}
\#\custombold{GCP} \#\custombold{Compute Engine}   \#\custombold{BigQuery} \#\custombold{Google Sign-In}  \#\custombold{Flask}  \#\custombold{Selenium}
\sectionsep

\section{other Projects}
\textbullet{} \href{https://github.com/zxingz/WebApp}{The-Software.Dev} \textbullet{} \href{https://github.com/zxingz/Workflow}{Airflow} \textbullet{} \href{https://github.com/zxingz/Monitoring}{Monitoring} \textbullet{} \href{https://github.com/zxingz/machine_comprehension}{Machine Comprehension- Google BERT}  \textbullet{} \href{https://github.com/zxingz/trader}{High Frequency Trading}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     RESEARCH
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\end{minipage} 
\end{document}